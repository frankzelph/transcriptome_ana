...
## Usage: Rscript DESeq2.R count_matrix_file design_table
##
...
DEG_analysis <- function(counts_file, design_table) {
	...
	res <- results(dds, name=tmp)
	
	# Filter rows with NA padj
	keep <- !is.na(res$padj)
	res <- res[keep,]
	# output results
	result_file <- paste(sub("condition_", "", tmp),".DEG",sep="")
...
DEG_analysis(counts_file, design_table)
